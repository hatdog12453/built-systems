...
@section('title', 'Chat with Coach - Fitness Coaching')

@section('content')
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-6">
        <a href="{{ route('client.dashboard') }}" class="text-indigo-600 hover:text-indigo-800">← Back to Dashboard</a>
    </div>

    <div class="bg-white rounded-lg shadow-lg">
        <!-- Chat Header -->
        <div class="bg-indigo-600 text-white px-6 py-4 rounded-t-lg">
            <h1 class="text-2xl font-bold">Chat with {{ $coach->full_name }}</h1>
            <p class="text-indigo-200 text-sm">{{ $coach->email }}</p>
        </div>
        ...
        <div id="messagesContainer" class="h-96 overflow-y-auto p-6 space-y-4 bg-gray-50">
            @forelse($messages as $message)
                <div class="flex {{ $message->sender_type === 'client' ? 'justify-end' : 'justify-start' }}">
                    <div class="max-w-xs lg:max-w-md px-4 py-2 rounded-lg {{ $message->sender_type === 'client' ? 'bg-indigo-600 text-white' : 'bg-white border border-gray-300 text-gray-900' }}">
                        @if($message->image)
                        ...
                        <p class="text-xs mt-1 {{ $message->sender_type === 'client' ? 'text-indigo-200' : 'text-gray-500' }}">
                            ...
                            @if($message->isRead() && $message->sender_type === 'client')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderMessages(messages) {
        ...
        messages.forEach(message => {
            ...
            const readStatus = isRead && message.sender_type === 'client' ? '<span class="ml-2">✓ Read</span>' : '';
